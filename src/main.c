...
static vec3 pathtrace(ray r, const scene *sc, glt_model *m, int depth) {
    (void)m;
    if (depth > 8) return v3(0, 0, 0);
    hit_record rec;
    if (!intersect_scene(r, sc, &rec)) return sc->bg_color;
    if (rec.emissive) return rec.albedo;
    vec3 emitted = v3(0, 0, 0);
    for (int i = 0; i < sc->nspheres; i++) {
        const sphere *l = &sc->spheres[i];
        ...
        hit_record srec;
        if (intersect_scene(shadow, sc, &srec) && srec.t < dist - 0.01f) continue;
        float pdf = 1.0f / (4.0f * 3.14159265f * l->radius * l->radius);
        float brdf_val = rec.albedo.x / 3.14159265f;
        emitted = vadd(emitted, vmul(l->emission, cos_theta * brdf_val / (pdf * dist * dist)));
    }
    vec3 new_dir = cosine_hemisphere(rec.normal);
    ray new_r = ray_new(rec.point, new_dir);
    vec3 indirect = pathtrace(new_r, sc, m, depth + 1);
...
static scene cornell_box(void) {
    scene sc = {0};
    sc.ambient = v3(0.1f, 0.1f, 0.1f);
    sc.bg_color = v3(0, 0, 0);
    sc.planes[sc.nplanes++] = (plane){v3(0, 0, 0), v3(0, 1, 0), v3(0.73f, 0.73f, 0.73f), 0, 0, v3(0,0,0)};
    sc.planes[sc.nplanes++] = (plane){v3(0, 5, 0), v3(0, -1, 0), v3(0.73f, 0.73f, 0.73f), 0, 0, v3(0,0,0)};
    sc.planes[sc.nplanes++] = (plane){v3(0, 0, -5), v3(0, 0, 1), v3(0.73f, 0.73f, 0.73f), 0, 0, v3(0,0,0)};
    sc.planes[sc.nplanes++] = (plane){v3(-3, 0, 0), v3(1, 0, 0), v3(0.63f, 0.065f, 0.065f), 0, 0, v3(0,0,0)};
    sc.planes[sc.nplanes++] = (plane){v3(3, 0, 0), v3(-1, 0, 0), v3(0.12f, 0.45f, 0.15f), 0, 0, v3(0,0,0)};
    sc.spheres[sc.nspheres++] = (sphere){v3(0, 4.9f, -2.5f), 0.5f, v3(1,1,1), 0, 1, v3(8, 8, 8)};
    sc.spheres[sc.nspheres++] = (sphere){v3(-1.2f, 1.2f, -2.5f), 1.2f, v3(0.73f, 0.73f, 0.73f), 0.1f, 0, v3(0,0,0)};
    sc.spheres[sc.nspheres++] = (sphere){v3(1.5f, 0.8f, -1.5f), 0.8f, v3(0.9f, 0.9f, 0.1f), 0.3f, 0, v3(0,0,0)};
    return sc;
}
...
int main(int argc, char **argv) {
    ...
    camera ca = cam_new(v3(0, 2.5f, 7), v3(0, 2.5f, -2.5f), v3(0, 1, 0), 45, (float)W / H);
    ...
    for (int y = 0; y < H; y++) {
        ...
        for (int x = 0; x < W; x++) {
            ...
            for (int s = 0; s < spp; s++) {
                float u = (x + randf()) / W;
                float v = (y + randf()) / H;
                ray r = cam_ray(ca, u, v);
                col = vadd(col, pathtrace(r, &sc, &m, 0));
            }
